const {
  STRING_MAX_LENGTH, IMAGE_PRIVACIES, IMAGE_PUBLIC, RELATION_FOLLOW, RELATION_TYPES,
...
describe('parseNonNegativeInteger', () => {
  ...
  const positiveIntegerfloats = [1.0, 7.0, 10.00, 1092.0, 17902.00000];
  const negativeIntegerfloats = positiveIntegerfloats.map((x) => -x);

  const positivefloats = [0.0009, 0.01, 0.60, 0.9, 12.7, 987.001, 120730.999];
  ...
  describe('number parameters', () => {
    describe('allowed values are returned as it is', () => {
      ...
      test('positive integers are allowed', () => {
        positiveIntegers.forEach(
          (value) => expect(parseNonNegativeInteger(value)).toBe(value),
        );
      });

      test('positive integer floats are allowed', () => {
        positiveIntegerfloats.forEach(
          (value) => expect(parseNonNegativeInteger(value)).toBe(value),
        );
      });
    });
  ...
  describe('string parameters', () => {
    describe('allowed values are converted into numbers', () => {
      test('zeroes are allowed', () => {
        zeroes.forEach(
          (value) => expect(parseNonNegativeInteger(value.toString())).toBe(0),
        );
      });

      test('positive integers are allowed', () => {
        positiveIntegers.forEach(
          (value) => expect(parseNonNegativeInteger(value.toString())).toBe(value),
        );
      });
      ...
      test('leading zeroes are allowed', () => {
        ...
        leadingZeroes.forEach(
          (value) => expect(parseNonNegativeInteger(value)).toBe(Number(value)),
        );
      });
    });
...
describe('string parsers', () => {
  describe('text type', () => {
    ...
    test('other than strings do throw error', () => {
      ...
      otherTypes.forEach(
        (value) => expect(() => parseTextType(value)).toThrow(ParseError),
      );
    });
  });
